...
<hr/>
@if(Auth::check())
<form class="col-md-12" method="POST" action="{{ route('saveCommentD')}}" >
 {{ csrf_field() }}	
	<input type="hidden" name="dieta_id" value="{{$dieta->id}}" required />
	<input type="hidden" name="receptor_id" value="{{$dieta->user_id}}" required />
...
@if(isset($dieta->comments))
	<div id=comment-list>
		@foreach($dieta->comments as $comment)
			<div class="comment-item col-xs-12 pull-left">
				<div class="panel panel-success comment-data">
					...
					<div class="panel-body">
						...
						@if(Auth::check() && (Auth::user()->id == $comment->user_id || Auth::user()->id == $dieta->user_id))
								    ...
								    <div class="modal-dialog">
								        <div class="modal-content">
								            ...
								            <div class="modal-footer">
								                ...
								                <a href="{{ url('/delete-commentd/'.$comment->id)}}" type="button" class="btn btn-danger">Eliminar</a>
								            </div>
								        </div>
								    </div>
								</div>
