...
<hr/>
@if(Auth::check())
<form class="col-md-12" method="POST" action="{{ route('saveCommentR')}}" >
 {{ csrf_field() }}	
	<input type="hidden" name="rutina_id" value="{{$rutina->id}}" required />
	<input type="hidden" name="receptor_id" value="{{$rutina->user_id}}" required />
...
@if(isset($rutina->comments))
	<div id=comment-list>
		@foreach($rutina->comments as $comment)
			<div class="comment-item col-xs-12 pull-left">
				<div class="panel panel-success comment-data">
					...
					<div class="panel-body">
						...
						@if(Auth::check() && (Auth::user()->id == $comment->user_id || Auth::user()->id == $rutina->user_id))
								    ...
								    <div class="modal-dialog">
								        <div class="modal-content">
								            ...
								            <div class="modal-footer">
								                ...
								                <a href="{{ url('/delete-commentr/'.$comment->id)}}" type="button" class="btn btn-danger">Eliminar</a>
								            </div>
								        </div>
								    </div>
								</div>
